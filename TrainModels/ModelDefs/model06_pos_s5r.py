...
... but only using positional features
...
model = {
    ...
    'nneg':  5,
    'nsrc':  'random',
    'notes': 'only positional features'
}

basename = 'model06_pos_s5r'
...
# only leaving the positional features:
X = np.concatenate([X[:,:3], X[:,-7:]], axis=1)
